...
fe_ini_vig_nivel = aux_tools.get_fecha_nivel('nivel_detalle', params.NIVELES_DETALLE, 'fecha_inicio_vigencia')
fe_fin_vig_nivel = aux_tools.get_fecha_nivel('nivel_detalle', params.NIVELES_DETALLE, 'fecha_fin_vigencia')
...
def prep_input_prima_directo (
    ...
    input_prima_directo = produccion_df.filter(
...
def prep_input_dcto_directo(
    ...
    input_dcto_directo = produccion_df.with_columns(
        pl.lit("dcto_directo").alias("tipo_insumo")).pipe(
            cruces.cruzar_param_contabilidad, param_contabilidad).pipe(
                cruces.cruzar_excepciones_50_50, excepciones_df).pipe(
                    cruces.cruzar_descuento, descuento_df
                )
    # la fecha de constitucion de reserva es la de emision del recibo
    input_dcto_directo = input_dcto_directo.filter(
        pl.col('fecha_contabilizacion_recibo') <= fe_valoracion
    ).with_columns(
        fe_ini_vig_nivel.alias("fecha_inicio_vigencia")
    ).with_columns(
...
def prep_input_prima_rea (
    ...
    input_prima_rea = cesion_rea_df.filter(
        pl.col('fecha_contabilizacion_recibo') <= fe_valoracion
    ).pipe(
        cruces.cruzar_param_contabilidad, param_contabilidad).pipe(
            cruces.cruzar_excepciones_50_50, excepciones_df
        ...
        (pl.col("valor_prima_cedida") * pl.col("porc_participacion_reasegurador")).alias("valor_base_devengo")
    )
...
def prep_input_dcto_rea(
    ...
    # cambia el tipo de insumo al descuento y aplica los cruces de parametros
    input_dcto_rea = cesion_rea_df.filter(
        pl.col('fecha_contabilizacion_recibo') <= fe_valoracion
    ).with_columns(
        pl.lit("dcto_rea_prop").alias("tipo_insumo")).pipe(
            cruces.cruzar_param_contabilidad, param_contabilidad).pipe(
                cruces.cruzar_excepciones_50_50, excepciones_df).pipe(
                    cruces.cruzar_descuento, descuento_df, True
    ).with_columns(
        fe_ini_vig_nivel.alias("fecha_inicio_vigencia")
    ).with_columns(
        pl.col("fecha_contabilizacion_recibo").alias("fecha_constitucion")
        ...
        ( prima_reconstruida_ced * pl.col("podto_comercial") * pl.col('porc_participacion_reasegurador')
...
def prep_input_comi_rea (
    ...
    input_comi_rea = comision_rea_df.filter(
        pl.col('fecha_contabilizacion_recibo') <= fe_valoracion
    ).pipe(
        cruces.cruzar_param_contabilidad, param_contabilidad).pipe(
            cruces.cruzar_excepciones_50_50, excepciones_df
        ...
        # los calculos deben estar abiertos por reasegurador
        (pl.col("valor_comision_rea") * pl.col("porc_participacion_reasegurador")).alias("valor_base_devengo")
    )
...
def prep_input_costo_con (
    ...
    if 'saldo_anterior' in costo_contrato.columns:
        base_devengo = pl.col('saldo_anterior')     # si es de un output anterior, ya viene abierto por reasegurador
        fe_inicio_devengo = pl.lit(fe_valoracion).dt.month_start()  # recalcula el devengo desde esta fecha
    # si se procesa por primera vez el recibo, parte de la base inicial
        ...
        base_devengo = pl.col('valor_costo_contrato') * pl.col("porc_participacion_reasegurador")
    ...
    # cruza con los datos de seguimiento del contrato
        cruzar_costo_seguim, seguimiento_costo, fe_valoracion
    ).with_columns(
    ...
    # se inicia a devengar en la fecha max entre constitucion, inicio cobertura rea o fecha de reinstalamento
        pl.max_horizontal([
            ...
            pl.max_horizontal([pl.coalesce([pl.col("fe_reinstalamento"), fe_inicio_devengo]), fe_inicio_devengo])
