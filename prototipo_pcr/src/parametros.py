...
RUTA_HISTORICO_DETERIORO = base_dir.parent / "inputs" / "historico_deterioro.xlsx"
...
# Rutas salidas
RUTA_SALIDA_DEVENGO = base_dir.parent / "output" / f"output_devengo_fluc_{FECHA_VALORACION.strftime('%d%m%Y')}.xlsx"
RUTA_SALIDA_CONTABLE = base_dir.parent / "output" / f"output_contable_{FECHA_VALORACION.strftime('%d%m%Y')}.xlsx"
...
CAMPOS_OUTPUT_CONTABLE = [
    ...
    "componente", "tipo_contabilidad", "regla_devengo", # nivel de agregacion y parametros
    "tipo_contrato", "cohorte", "anio_liberacion", "transicion", # nivel de agregacion
    "fecha_inicio_periodo", "fecha_valoracion", # fecha de inicio y fin del periodo de valoracion
    "fecha_constitucion", "fecha_inicio_devengo", "fecha_fin_devengo", "valor_base_devengo", # parametros devengo
    "dias_devengados", "dias_no_devengados", "control_suma_dias", # solo estan para devengo diario
    "dias_constitucion", "dias_liberacion", "valor_devengo_diario", # solo estan para devengo diario
]
CAMPOS_OUTPUT_5050 = [
    "mes_constitucion", "mes_ini_liberacion", "mes_fin_liberacion", # solo estan para devengo 5050
]
